...
// TEMPORÁRIO: recalcula dentroSLA dos tickets 2024-2025 baseado na planilha Pipefy
...
// Lista da planilha "Base Pipefy" — STATUS="Fora da SLA"
const FORA_SLA = [
...
module.exports = async function handler(req, res) {
  ...
  // 1. Reset: primeiro coloca TODOS os tickets 2024-2025 do Pipefy como dentroSLA=true
  // (porque o syncPipefy do admin sobrescreve com base errada)
  ...
  snap.forEach(doc => {
    ...
    if (ano === '2024' || ano === '2025') {
      tickets2024_2025.push({ docId: doc.id, ...t, _ano: ano });
    }
  });

  // 2. Indexar planilha
  const planilhaIdx = new Map();
  FORA_SLA.forEach(item => {
    const key = `${norm(item.nome)}|${item.dataAbertura}`;
    planilhaIdx.set(key, item);
  });

  // 3. Classificar todos
  const reset_to_true = []; // tickets que estavam false e devem voltar a true
  const marcar_false = []; // tickets da planilha que devem ficar false
  const ja_corretos_true = [];
  const ja_corretos_false = [];
  ...
  tickets2024_2025.forEach(t => {
    ...
    if (naLista) {
      ...
      if (t.dentroSLA === true) {
        marcar_false.push({ docId: t.docId, titulo: t.titulo, data: dataKey(t.data_abertura), motivo: naLista.motivo });
      } else {
        ja_corretos_false.push({ docId: t.docId, titulo: t.titulo });
      }
    } else {
      // Não está na lista de "Fora" → deve ser true
      if (t.dentroSLA === false) {
        reset_to_true.push({ docId: t.docId, titulo: t.titulo, data: dataKey(t.data_abertura) });
      } else {
        ja_corretos_true.push(t.docId);
      }
    }
  });

  const naoEncontradosNaPlanilha = Array.from(planilhaIdx.values());

  const resumo = {
    modo: dryRun ? 'DRY-RUN (não aplicado)' : 'APLICADO',
    total_2024_2025: tickets2024_2025.length,
    estado_atual: {
      true: ja_corretos_true.length + reset_to_true.length,
      false: ja_corretos_false.length + marcar_false.length,
    },
    acoes: {
      reset_para_true: reset_to_true.length,
      marcar_como_false: marcar_false.length,
      ja_corretos: ja_corretos_true.length + ja_corretos_false.length,
    },
    nao_encontrados_na_planilha: naoEncontradosNaPlanilha.length,
    taxa_sla_depois: tickets2024_2025.length > 0
      ? `${(((ja_corretos_true.length + reset_to_true.length) / tickets2024_2025.length) * 100).toFixed(1)}%`
      : '0%',
  };

  // 4. Aplicar se não for dry-run
  if (!dryRun) {
    const now = new Date();
    // Reset to true
    if (reset_to_true.length > 0) {
  ...
  res.status(200).json({
    ...
    resumo,
    marcar_false,
    reset_to_true: reset_to_true.slice(0, 50),
    nao_encontrados: naoEncontradosNaPlanilha,
  });
};
